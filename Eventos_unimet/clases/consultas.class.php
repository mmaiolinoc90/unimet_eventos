...
////////		Public Methods
	...
	*/
	public function getRecords ($where_str=false, $order_str=false, $count=false, $start=0){
		...
		$query ="SELECT $campos FROM {$this->table} $where $order $limit"; //echo $query."<br><br>";
	...
	*/
	...
	public function insertRecord ($data){
		...
		$data =implode ("', '", $data);
		$query ="INSERT INTO {$this->table} ($campos) VALUES ($sysData '$data')"; //echo $query."<br>";
		mysql_query ($query);
	...
	public function updateRecord ($id, $data){
		...
		$datos =implode (", ", $datos);
		$query = "UPDATE {$this->table} SET $datos WHERE {$this->campoId}=$id"; //echo $query."<br>";
		mysql_query ( $query );
		return $this->validateOperation ();
	}
	public function deleteRecord ($id){
		mysql_query ("DELETE FROM {$this->table} WHERE {$this->campoId}=$id");
		return $this->validateOperation ();
	}
	
	public function deleteRecords ($where){
		mysql_query ("DELETE FROM {$this->table} WHERE $where");
		return $this->validateOperation ();
	}	
	public function numPag ($where_str=false, $count=0){
		$where =$where_str ? "WHERE $where_str" : "";
		$query ="SELECT COUNT({$this->campoId}) FROM {$this->table} $where"; //echo $query;
		$consQ =mysql_query ($query);
